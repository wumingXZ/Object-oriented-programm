...
> **"共享"到底是什么意思？** `static int count` 在整个程序中只有**一份**。创建 100 个 `Student` 对象，这 100 个对象共用同一个 `count`。每个对象有自己的 `name`（普通成员），但 `count` 是大家共用的。就像班级公告栏上的"全班人数"——不是贴在每个学生课桌上，而是挂在墙上，谁都可以看、可以改。
>
> ```cpp
> Student a("Alice"), b("Bob");
> // a.name 是 "Alice"，b.name 是 "Bob" —— 各自独立
> // a.count 和 b.count 是同一个东西 —— 共享
> // sizeof(Student) 只包含 name 的大小，不包含 count
> ```
...
| 归属 | 每个对象独有一份 | 整个类共享一份 |
| 内存位置 | 对象内部（堆/栈） | 静态存储区 |
| 初始化 | 构造函数中 | **必须在类外定义** `int C::x = 0;` |
| 访问 | 必须通过对象 `obj.x` | 推荐 `类名::x` |
| 生命周期 | 随对象创建/销毁 | 程序启动到结束 |
...
| 访问非静态成员 | ✅ | ❌（除非通过参数传入对象） |
| 访问静态成员 | ✅ | ✅ |
| 可否 `const` | ✅ | ❌（没有 `this`，无意义） |
| 可否 `virtual` | ✅ | ❌（多态依赖对象） |
| 调用方式 | `obj.func()` | `类名::func()`（推荐） |

> **典型使用场景**：
> - **计数器**：统计类创建了多少个实例（如上例）
> - **工厂方法**：`static unique_ptr<Foo> Foo::create(...)` 返回新对象
> - **工具函数**：与类相关但不依赖具体对象，如 `Math::sqrt()`
> - **单例模式**：`static Instance& getInstance()` 返回全局唯一实例
>
> **注意事项**：静态成员函数可以访问私有静态成员，遵守正常的访问控制规则。C++17 起可以用 `inline static int count = 0;` 在类内直接初始化，省去类外定义。
